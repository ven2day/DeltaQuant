...
class IdempotencyStore:
    ...
    def seen(self, key: str) -> dict[str, Any] | None:
        ...
        try:
            row = session.query(IdempotencyRecord).filter_by(key=key).first()
    ...
    def record(self, key: str, result: dict[str, Any]) -> None:
        ...
        try:
            ...
            if row is None:
                session.add(
                    IdempotencyRecord(key=key, result_json=json.dumps(result), created_at=now)
                )
            else:
            ...
            count = session.query(IdempotencyRecord).count()
