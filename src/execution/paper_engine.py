...
class PaperPositionRecord(Base):
    ...
    Only structural/entry fields are persisted — mark-to-market fields (current_price,
    unrealized_pnl, mae/mfe, highest/lowest_price) are cheap to recompute from live prices
    via update_positions_pnl() after a restart, so persisting them on every price tick
    would be pure write overhead for no durability benefit.
    """
...
class LocalPaperEngine:
    ...
    def _load_state(self) -> None:
        ...
        try:
            wallet = session.query(PaperWalletRecord).filter_by(id=1).first()
            if wallet is None:
            ...
            for row in session.query(PaperPositionRecord).all():
    ...
    def _persist_after_trade(self, new_order: Order) -> bool:
        ...
        try:
            wallet = session.query(PaperWalletRecord).filter_by(id=1).first()
            if wallet is None:
                wallet = PaperWalletRecord(id=1, balance=self.balance, initial_balance=self.initial_balance)
                session.add(wallet)
            ...
            session.query(PaperPositionRecord).delete()
    ...
    def place_order(
        ...
        *,
        ...
        if idempotency_key:
            session = self._session()
            try:
                prior = session.query(PaperOrderRecord).filter_by(
        ...
        while remaining > 0:
            matching = next(
                (
                    position
                    for position in self.positions.values()
                    if position.symbol == symbol and position.side == opposite
                    and (not position_id or position.position_id == position_id)
                ),
                None,
            )
    ...
    def get_closed_trade_history(self, limit: int = 250) -> list[dict[str, Any]]:
        ...
        session = self._session()
        try:
            orders = session.query(PaperOrderRecord).order_by(PaperOrderRecord.timestamp).all()
        finally:
            session.close()
        ...
        for order in orders:
            ...
            opposite = "SELL" if side == "BUY" else "BUY"
            matching = next(
                (position for position in open_positions if position["symbol"] == order.symbol
    ...
    def reset(self) -> None:
        ...
        try:
            session.query(PaperPositionRecord).delete()
            session.query(PaperOrderRecord).delete()
            session.query(PaperWalletRecord).delete()
            session.commit()
        except Exception:
