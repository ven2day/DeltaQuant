...
Drawdown tracking (``src.risk.guards.DrawdownTracker``) intentionally stays
session-scoped and reseeded from live equity on each restart (see the seeding comment
in ``scripts/run_live_trading.py``) — that behavior is unrelated to daily limits and is
...
class DailyRiskRecord(Base):
    ...
    __table_args__ = (UniqueConstraint("trade_date", name="uq_daily_risk_state_date"),)
...
class DailyRiskStore:
    ...
    def _get_or_create(self, session: Session, trade_date: date) -> DailyRiskRecord:
        record = session.query(DailyRiskRecord).filter_by(trade_date=trade_date).first()
        if record is None:
