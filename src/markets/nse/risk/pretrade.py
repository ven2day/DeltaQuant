    ...
    def evaluate(
        ...
        *,
        ...
        current_exposure = sum(
            float(position.quantity) * float(position.current_price or position.entry_price)
        ...
        if position_pct > max_position_pct:
            reasons.append(f"Position notional {position_pct:.1%} exceeds {max_position_pct:.1%}.")
        ...
        if daily_loss_limit is not None:
            remaining_daily_loss = max(
                0.0,
                float(daily_loss_limit) + min(0.0, float(realized_daily_pnl)),
            )
            buffered_remaining = remaining_daily_loss * min(1.0, max(0.0, float(daily_loss_buffer)))
            if projected_risk > buffered_remaining:
                reasons.append(
                    f"Projected stop risk Rs.{projected_risk:,.2f} exceeds buffered "
                    f"remaining daily-loss capacity Rs.{buffered_remaining:,.2f}."
                )
