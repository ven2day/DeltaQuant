...
class TradeRecord(Base):
    ...
    market = Column(String(16), default="NSE", nullable=False, index=True)
    provider = Column(String(24), default="DHAN", nullable=False, index=True)
    broker_account_ref = Column(String(128), default="", nullable=False)
...
class TradeJournal:
    ...
    def _initialize_db(self) -> None:
        ...
        try:
            ...
            try:
                columns = {
                    column["name"] for column in inspect(self._engine).get_columns("trade_journal")
                }
                with self._engine.begin() as connection:
                    ...
                    if "run_id" not in columns:
                        connection.execute(
                            text(
                                "ALTER TABLE trade_journal ADD COLUMN run_id VARCHAR(80) DEFAULT ''"
                            )
                        )
                    ...
                    if "provider" not in columns:
                        connection.execute(
                            text(
                                "ALTER TABLE trade_journal ADD COLUMN "
                                "provider VARCHAR(24) DEFAULT 'DHAN'"
                            )
                        )
                    if "broker_account_ref" not in columns:
                        connection.execute(
                            text(
                                "ALTER TABLE trade_journal ADD COLUMN "
                    ...
                    decision_columns = {
                        column["name"]
                        for column in inspect(self._engine).get_columns("decision_logs")
                    }
                    if "namespace" not in decision_columns:
                        connection.execute(
                            text(
                                "ALTER TABLE decision_logs ADD COLUMN "
                                "namespace VARCHAR(40) DEFAULT 'paper_market_data'"
                            )
                        )
            except Exception:
    ...
    def record_trade(
        ...
        *,
        ...
        record = TradeRecord(
            ...
            market=trade.get("market", state.get("market", "NSE")),
            provider=trade.get(
                "market_data_provider", state.get("market_data_provider", "DHAN")
            ),
            broker_account_ref=trade.get("broker_account_ref", ""),
            ...
            decision_chain=json.dumps(
                {
                    **self._extract_decision_chain(state),
                    "strategy_eligibility": {
                        ...
                        "model_version": trade.get(
                            "model_version", trade.get("strategy_version")
                        ),
                        "validation_status": trade.get(
                            "eligibility_status", trade.get("validation_status")
                        ),
                        "current_regime": trade.get("current_regime", state.get("regime")),
                        "regime_policy": trade.get("regime_policy"),
                        "original_confidence": trade.get("original_confidence"),
                        "regime_adjusted_confidence": trade.get(
                            "regime_adjusted_confidence"
                        ),
                        "registry_decision": trade.get("registry_decision"),
                        "registry_reason": trade.get("registry_reason"),
                    },
                    "ml": {
                        "confidence": trade.get("ml_probability"),
                        "status": trade.get("ml_status"),
                        "model_version": trade.get("ml_model_version"),
                    },
                    "risk_decision": trade.get("risk_result", {}),
                    "final_outcome": trade.get("decision_chain", {}).get("final"),
    ...
    def get_trades_by_date(
        ...
        try:
            query = self._session.query(TradeRecord).filter(
                TradeRecord.namespace == self.namespace,
    ...
    def get_trades_by_strategy(self, strategy: str) -> list[dict[str, Any]]:
        ...
        try:
            records = (
                self._session.query(TradeRecord)
                .filter_by(strategy=strategy, namespace=self.namespace)
    ...
    def get_performance_summary(
        ...
        """
        try:
            query = self._session.query(TradeRecord).filter_by(
                status="closed", namespace=self.namespace
            )
