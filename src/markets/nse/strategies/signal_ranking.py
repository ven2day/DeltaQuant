...
from src.core.candidates import SignalType, TradingSignal
from src.markets.nse.universe.sectors import get_stock_sector  # noqa: F401
...
def rank_signals(
    ...
    *,
    ...
    for signal in signals:
        ...
        estimated_probability = min(0.95, max(0.05, estimated_probability + discovered_tilt))
        expected_r = estimated_probability * float(signal.risk_reward_ratio) - (
...
def technical_pre_rank(
    signals: list[TradingSignal],
    performance_tracker: PerformanceTrackerLike,
    *,
    lookback_days: int = 90,
) -> list[RankedSignal]:
    """Cheap Stage-1 score using only technical and persisted outcome evidence."""
    return rank_signals(
        signals,
        {},
        performance_tracker,
...
def select_stage1_candidates(
    ...
    *,
    ...
    """Select every unique symbol/timeframe that passed technical quality gates.

    One representative per symbol/timeframe is intentional: every strategy firing on
    the same settled candle would otherwise retrain the identical model and consume
    downstream candidate-evaluation capacity without adding independent evidence.
    """
    # ``max_candidates`` and ``max_per_sector`` remain in the signature for API
    # compatibility. Ranking controls processing order; it is not an admission rule.
    # Portfolio/risk limits are applied later by deterministic risk compliance.
    selected: list[RankedSignal] = []
    seen_pairs: set[tuple[str, str]] = set()
    for item in ranked:
        symbol = item.signal.symbol
        key = (symbol, item.signal.timeframe.value)
...
def select_diversified_signals(
    ...
    *,
    ...
    """Preserve all unique ranked opportunities; ranking is prioritization only."""
