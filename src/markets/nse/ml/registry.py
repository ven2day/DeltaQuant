"""NSE-bound model registry; cross-market records fail closed."""

from src.core.ml import ModelGrain, ModelRecord, ModelRegistry, ModelStatus


class NSEModelRegistry(ModelRegistry):
    @staticmethod
    def _check(grain: ModelGrain) -> None:
        if grain.market.strip().upper() != "NSE":
            raise ValueError("NSE model registry cannot access another market")
    ...
    def list_records(self, *, market: str | None = None) -> list[ModelRecord]:
        if market is not None and market.strip().upper() != "NSE":
            raise ValueError("NSE model registry cannot list another market")
        return super().list_records(market="NSE")


__all__ = ["NSEModelRegistry"]
