...
def get_valid_access_token() -> str | None:
    ...
    cache_path = project_path(settings.dhan_token_cache_file)
...
def get_dhan_client(client_id: str, access_token: str) -> Any:
    """Construct the installed DhanHQ v2 client from its credential context.

    ``dhanhq>=2.2`` accepts one :class:`DhanContext` argument rather than the
    legacy ``(client_id, access_token)`` positional pair.  The context owns the
    pooled HTTP client and supplies both required authentication headers.
    """
