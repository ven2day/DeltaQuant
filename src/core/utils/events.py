...
class TradingEvent:
    ...
    @property
    def topic(self) -> str:
        """Market-owned topic name used by external queue adapters."""
        return f"{self.market.strip().lower()}.{self.event_type.value}"
...
async def emit_candle_settled(
    ...
    *,
    ...
    """Emit one material settled-candle event (never emitted for timer wake-ups)."""
    await get_event_bus().publish(
        TradingEvent(
            event_type=EventType.CANDLE_SETTLED,
            data={
                "symbol": symbol,
                "timeframe": timeframe,
...
async def emit_signal_generated(
    signal: dict[str, Any], *, market: str = "NSE", provider: str = "", runtime_id: str = ""
) -> None:
...
async def emit_order_filled(
    order: dict[str, Any], *, market: str = "NSE", provider: str = "", runtime_id: str = ""
) -> None:
