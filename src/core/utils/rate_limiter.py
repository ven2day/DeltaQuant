...
class RateLimiter:
    ...
    def available_tokens(self) -> float:
        """Get current available tokens at stable millitoken precision.

        A property read occurs microseconds after construction and therefore otherwise
        exposes meaningless monotonic-clock refill dust (for example 1.000004 tokens).
        Acquisition keeps full precision; only this diagnostic snapshot is rounded.
        """
        self._refill_tokens()
...
# (`src.core.utils.rate_limiter._groq_limiter = None`) keep working unchanged.
