...
from src.core.strategies.config import PRODUCTION_STRATEGY_NAMES, ProductionStrategyConfig
from src.core.strategies.production import evaluate_production_strategy
...
class StrategyType(Enum):
    ...
    # trades until they clear scripts/validate_strategy.py and receive paper/live eligibility --
    ...
    SUPERTREND_ADX_EMA = "supertrend_adx_ema"
    MACD_TREND_CONTINUATION = "macd_trend_continuation"
    BOLLINGER_RSI_MEAN_REVERSION = "bollinger_rsi_mean_reversion"
    VWAP_MEAN_REVERSION = "vwap_mean_reversion"
    OPENING_RANGE_BREAKOUT = "opening_range_breakout"
    RELATIVE_STRENGTH_MOMENTUM = "relative_strength_momentum"
    # Point of Control / Value Area breakout: unlike every strategy above (all derived
    # purely from price), this reads volume-at-price -- genuinely different information,
    # not another price-only indicator restated. Approximated from OHLCV bars (no tick
    # data available from Dhan); see IndicatorConfig.volume_profile_* / engine.py's
    # _latest_volume_profile. Like every other strategy here, it admits no live/paper
    # trades until it clears scripts/validate_strategy.py.
    VOLUME_PROFILE_BREAKOUT = "volume_profile_breakout"
    # EMA20/50 + RSI/MACD + volume/candle-quality confluence (breakout, pullback, or
    # reversal setup), adapted from a user-supplied TradingView Pine Script. Long-only,
    # matching the source script. Like every other strategy here, it admits no live/
    # paper trades until it clears scripts/validate_strategy.py.
    EMA_TREND_CONFLUENCE = "ema_trend_confluence"
    # High-volume-confirmed pivot support/resistance zone reactions (breakout,
    # breakdown, hold, or flipped-zone retest), adapted from a user-supplied
    # TradingView Pine Script. Like every other strategy here, it admits no live/
    # paper trades until it clears scripts/validate_strategy.py.
    PIVOT_VOLUME_SR_ZONES = "pivot_volume_sr_zones"


class TradeHorizon(Enum):
    """Which decision and risk horizon a signal belongs to.
    ...
    compatibility with every pre-existing signal); SCALP is the 5m/15m execution
    horizon. Eligibility artifacts intentionally do not key on this value; prediction
    semantics and deterministic risk limits may still remain horizon-specific.
    """
...
class TradingSignal:
    ...
    # Explicit runtime lineage. The live orchestrator overwrites this with its current
    # mode before a signal reaches LangGraph or execution; the default preserves older
    # constructors and direct unit tests.
    execution_mode: str = "market_paper"

    # Reproducible multi-strategy decision lineage.  Defaults keep historical
    # constructors compatible; the event runtime populates these from exact registry
    # and settled-candle state before a signal can reach validation/execution.
    ...
    strategy_consensus_identity: str = ""
    ...
    model_version: str = ""
    eligibility_status: str = "UNVALIDATED"
    eligibility_environment: str = ""
    current_regime: str = "unknown"
    regime_policy: str = "ALLOW"
    original_confidence: float | None = None
    regime_adjusted_confidence: float | None = None
    registry_decision: str = "NOT_EVALUATED"
    registry_reason: str = ""
    registry_execution_allowed: bool = False
    registry_qwen_allowed: bool = False
    reason_codes: list[str] = field(default_factory=list)
    supporting_metrics: dict[str, float | int | str | bool] = field(default_factory=dict)
    score_components: dict[str, float] = field(default_factory=dict)
    technical_score_is_probability: bool = False
    market: str = "NSE"
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "strategy_consensus_identity": self.strategy_consensus_identity,
            ...
            "eligibility_environment": self.eligibility_environment,
            "current_regime": self.current_regime,
            "regime_policy": self.regime_policy,
            "original_confidence": self.original_confidence,
            "regime_adjusted_confidence": self.regime_adjusted_confidence,
            "registry_decision": self.registry_decision,
            "registry_reason": self.registry_reason,
            "registry_execution_allowed": self.registry_execution_allowed,
            "registry_qwen_allowed": self.registry_qwen_allowed,
            "reason_codes": list(self.reason_codes),
            "supporting_metrics": dict(self.supporting_metrics),
            "score_components": dict(self.score_components),
            "technical_score_is_probability": self.technical_score_is_probability,
        }
...
class SignalEngine:
    ...
    production_config: ProductionStrategyConfig = field(
        default_factory=ProductionStrategyConfig.defaults,
        repr=False,
    )
    ...
    def generate_signals(
        ...
        *,
        ...
        for strategy in active_strategies:
            signal = self._run_strategy(indicators, strategy, market_relative=market_relative)
    ...
    def _run_strategy(
        ...
        *,
        ...
        elif strategy.value in PRODUCTION_STRATEGY_NAMES:
            if not self.production_config.eligible(strategy.value, indicators.timeframe.value):
                return None
            return self._production_strategy(indicators, strategy, market_relative)
    ...
    def _production_strategy(
        ...
        decision = evaluate_production_strategy(
            strategy.value,
            ind,
        ...
        if decision is None:
            return None
        signal_type = SignalType(decision.direction)
        strength = (
            SignalStrength.STRONG
            if decision.technical_score >= 0.75
            else SignalStrength.MODERATE
            if decision.technical_score >= 0.50
        ...
        return self._create_signal(
            ...
            supporting_metrics=dict(decision.supporting_metrics),
        )
    ...
    def _ema_cci_strategy(self, ind: IndicatorResult) -> TradingSignal | None:
        """
        EMA(200) + CCI(14) strategy, from
        research/nse/external_strategies/TraderVersity-EMACCI.tpl (a MetaTrader chart
    ...
    def _create_signal(
        ...
        *,
        ...
        supporting_metrics: dict[str, float | int | str | bool] | None = None,
    ) -> TradingSignal:
        ...
        if technical_score is None:
            ...
            # Production strategy confidence is the explicitly component-weighted
            # technical score. It is evidence, never represented as a probability.
            final_confidence = round(max(0.0, min(1.0, technical_score)), 6)
        ...
        return TradingSignal(
            ...
            technical_score_is_probability=False,
        )
